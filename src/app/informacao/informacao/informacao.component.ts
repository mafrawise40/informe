      ...
      this.informacaoService.getById(params.id).subscribe({
        next: (v) => {
          ...
          if (v.marcadores[0] && v.marcadores[0].endereco) {
            console.log("Atualizando o editarExisteMarcador")
            ...
            this.atualizarMarker(this.informacaoDto.marcadores[0].latitude, this.informacaoDto.marcadores[0].longitude ,17);
...
salvar() {
  ...
  //salvar a informacao
  console.log(this.informacaoDto);
...
processarUpload() {
  this.informacaoService.uploadImagens(this.formData, this.informacaoDto.id).subscribe({
    next: (v) => { },
    error: (e) => {
...
atualizarMarker(lat: any, lng: any, zoom: number) {
  this.markers = [];
  this.markers.push(
    {
      lat: lat,
...
getLocation() {
  ...
  this.router.params.subscribe(params => {
     ...
     if (!this.editar && params && params.latitude != 0) {
      ...
      this.atualizarMarker(params.latitude, params.longitude ,12);
      ...
      if (navigator.geolocation) {
          ...
          this.atualizarMarker(position.coords.latitude, position.coords.longitude , 17);
